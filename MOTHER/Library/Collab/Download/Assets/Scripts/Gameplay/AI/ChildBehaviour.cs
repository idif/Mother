...
public class ChildBehaviour : CharacterBehaviour {
	...
	new protected void Awake(){
		...
		if(traits.Contains(ChildTrait.Random))
		{
			...
			firstTrait = (ChildTrait)Random.Range(0, ChildTrait.GetNames(typeof(ChildTrait)).Length + 1);
		...
		{
			if (isBoy) {
				int myNewMaleName = (Random.Range (0, ChildrenChoices.Self ().randomMaleNames.Length + 1));
				childName = ChildrenChoices.Self ().randomMaleNames [myNewMaleName];
			} else {
				int myNewFemaleName = (Random.Range (0, ChildrenChoices.Self ().randomFemaleNames.Length + 1));
